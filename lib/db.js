...
function createDb(pool, deps) {
  ...
  async function runMigrations() {
    const statements = [
      ...
      // Migration: ride_upcoming_notified_at for dedup of upcoming ride notifications
      `ALTER TABLE rides ADD COLUMN IF NOT EXISTS ride_upcoming_notified_at TIMESTAMPTZ;`
    ];
  ...
  async function seedNotificationPreferences(userId, role) {
    // Filter event types by role — each role only sees their own event types
    const types = role
      ? NOTIFICATION_EVENT_TYPES.filter(e => e.targetRole === role)
      : NOTIFICATION_EVENT_TYPES.filter(e => e.targetRole === 'office');

    const officeEmailOnByDefault = ['driver_tardy', 'rider_no_show', 'rider_approaching_termination', 'rider_terminated', 'daily_summary', 'driver_missed_ride'];

    for (const evt of types) {
      for (const channel of ['email', 'in_app']) {
        let defaultEnabled;
        if (channel === 'in_app') {
          defaultEnabled = true; // in-app always ON by default for all roles
        } else if (role === 'driver') {
          defaultEnabled = false; // Drivers: all email OFF by default
        } else if (role === 'rider') {
          defaultEnabled = true; // Riders: all email ON by default
        } else {
          defaultEnabled = officeEmailOnByDefault.includes(evt.key);
        }
        await query(`
